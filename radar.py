...
class Radar:
    ...
    def render(self):
        ...
        self.runway_start = radar_pixel(
            (self.data["runway"]["start_lat"],
             self.data["runway"]["start_long"]), self.data, self.scale)
        self.runway_end = radar_pixel(
            (self.data["runway"]["end_lat"], self.data["runway"]["end_long"]),
            self.data, self.scale)
        pygame.draw.line(self.surface, (255, 255, 255), self.runway_start,
                         self.runway_end, width=3)
        for line in self.data["lines"]:
            pygame.draw.line(
                self.surface, (255, 255, 255), radar_pixel(
                    (line["start_lat"], line["start_long"]),
                    self.data, self.scale), radar_pixel(
                        (line["end_lat"], line["end_long"]),
                        self.data, self.scale))
